...
```
docker pull kojitechs/dockerintro-springboot-helloworld-rest-api:1.0.0-RELEASE
```

## Step-3: Run the downloaded Docker Image & Access the Application
- Copy the docker image name from Docker Hub
```
docker run --name app1 -p 80:8080 -d kojitechs/dockerintro-springboot-helloworld-rest-api:1.0.0-RELEASE
...
docker run --name kube1 -p 80:80 --platform linux/amd64 -d  kojibello/kubenginx:1.0.0
http://localhost

## Sample Output
kalyanreddy@Kalyans-Mac-mini-2 ~ % docker run --name kube1 -p 80:80 --platform linux/amd64 -d  kojibello/kubenginx:1.0.0
